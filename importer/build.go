...
func (ctxt *Context) Import(path string, srcDir string, mode ImportMode) (*Package, error) {
	...
	if IsLocalImport(path) {
		if srcDir == "" {
			return nil, fmt.Errorf("import %q: import relative to unknown directory", path)
		}
		...
		if ctxt.GOROOT != "" {
			root := filepath.Join(ctxt.GOROOT, "src")
		...
		for i, root := range all {
			rootsrc := filepath.Join(root, "src")
			if sub, ok := hasSubdir(rootsrc, p.Dir); ok && !inTestdata(sub) {
				// We found a potential import path for dir,
				// but check that using it wouldn't find something
				// else first.
				if ctxt.GOROOT != "" {
					if dir := filepath.Join(ctxt.GOROOT, "src", sub); isDir(dir) {
						p.ConflictDir = dir
						goto Found
					}
				}
		...
		if ctxt.GOROOT != "" {
			dir := filepath.Join(ctxt.GOROOT, "src", path)
			isDir := isDir(dir)
			binaryOnly = !isDir && mode&AllowBinary != 0 
			if isDir || binaryOnly {
				p.Dir = dir
				p.Goroot = true
				p.Root = ctxt.GOROOT
		...
		return nil, fmt.Errorf("cannot find package %q in any of:\n%s", path, strings.Join(paths, "\n"))
	}

Found:
	if p.Root != "" {
		p.SrcRoot = filepath.Join(p.Root, "src")
		p.PkgRoot = filepath.Join(p.Root, "pkg")
		p.BinDir = filepath.Join(p.Root, "bin")
	}
	...
	for _, d := range dirs {
		...
		if mode&ImportComment != 0 {
			...
			if line != 0 {
				...
				if err != nil {
					return nil, fmt.Errorf("%s:%d: cannot parse import comment", filename, line)
				}
				if p.ImportComment == "" {
					p.ImportComment = com
					firstCommentFile = name
				} else if p.ImportComment != com {
					return nil, fmt.Errorf("found import comments %q (%s) and %q (%s) in %s", p.ImportComment, firstCommentFile, com, name, p.Dir)
				}
			}
		}
		...
		for _, decl := range pf.Decls {
			...
			for _, dspec := range d.Specs {
				...
				if path == "C" {
					if isTest {
						return nil, fmt.Errorf("use of cgo in test %s not supported", filename)
					}
					cg := spec.Doc
